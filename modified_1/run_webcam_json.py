    ...
    while cap.isOpened():
        try:
            ...
            if (result1 and result2) > threshold:
                ...
                cv2.putText(image, "FPS: %f" % (1.0 / (time.time() - fps_time)),
                            (10, 10),  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(image, "Mind Upper", (100, 50),  cv2.FONT_HERSHEY_SIMPLEX, 2.35, (0,0,255), 5)
                cv2.imshow('tf-pose-estimation Webcam', image)
                cv2.imshow('tf-pose-estimation Video', image2)
                fps_time = time.time()
                ...
                cv2.putText(image, "FPS: %f" % (1.0 / (time.time() - fps_time)),
                            (10, 10),  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                cv2.putText(image, "Mind Lower", (100, 50),  cv2.FONT_HERSHEY_SIMPLEX, 2.35, (0,0,255), 5)
                cv2.imshow('tf-pose-estimation Webcam', image)
                cv2.imshow('tf-pose-estimation Video', image2)
                fps_time = time.time()
            else:
